...
const outputFile = path.resolve(
  __dirname,
  '../packages/BigScreenDesign/LocalSourceDialog/imageConfig.js'
)
...
// 读取目录
const dirs = fs.readdirSync(imagesDir).filter((file) => {
...
// 遍历每个目录下的文件
dirs.forEach((dir) => {
  const files = fs.readdirSync(path.join(imagesDir, dir)).filter((file) => {
    const ext = path.extname(file).toLowerCase()
    return (
      ['.svg', '.png', '.jpg', '.jpeg'].includes(ext) && !file.startsWith('.')
    ) // 排除隐藏文件
  })
