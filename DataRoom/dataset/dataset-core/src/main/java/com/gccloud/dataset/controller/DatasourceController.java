...
public class DatasourceController {
    ...
    public R<PageVO<DatasourceEntity>> getPage(DatasourceSearchDTO searchDTO) {
        ...
        if (StringUtils.isBlank(tenantId)) {
            log.error("租户ID为空，不允许查询数据源");
            return (R<PageVO<DatasourceEntity>>) R.error(403,"租户ID为空，不允许查询数据源");
        }
    ...
    public R<List<DatasourceEntity>> getList(DatasourceSearchDTO searchDTO) {
        ...
        if (StringUtils.isBlank(tenantId)) {
            log.error("租户ID为空，不允许查询数据源");
            return (R<List<DatasourceEntity>>) R.error(403,"租户ID为空，不允许查询数据源");
        }
    ...
    public R<String> add(@RequestBody DatasourceDTO datasource) {
        ...
        // 检查租户ID是否为空
        if (StringUtils.isBlank(tenantId)) {
            log.error("租户ID为空，不允许新增数据源");
            return (R<String>) R.error(403,"租户ID为空，不允许新增数据源");
        }
